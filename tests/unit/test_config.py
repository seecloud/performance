...
class ConfigTestCase(test.TestCase):
    ...
    @mock.patch("performance.config.json")
    @mock.patch("performance.config.os.environ")
    @mock.patch("performance.config.open", create=True)
    def test_get_config(self, mock_open, mock_environ, mock_json):
        ...
        cfg = config.get_config()
        self.assertEqual(
            {"flask": {"DEBUG": False, "HOST": "0.0.0.0", "PORT": 5010}},
            cfg)
